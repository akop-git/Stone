...
function CanvasPage({ tasks, setTasks, onAdd, onLongPress, viewport, setViewport }) {
  ...
  const jumpToZone = (zone) => {
    // Find stones actually placed in this zone
    const stonesInZone = tasks.filter(t => {
      const tx = t.x ?? 0
      const ty = t.y ?? 0
      return tx >= zone.x && tx < zone.x + zone.w && ty >= zone.y && ty < zone.y + zone.h
    })

    if (stonesInZone.length === 0) {
      // No stones — center the zone geometrically
    ...
    // Compute bounding box around the stones (stones are ~180×80)
    const STONE_W = 180
    const STONE_H = 80
    const PADDING = 80
    const minX = Math.min(...stonesInZone.map(t => t.x ?? 0)) - PADDING
    const minY = Math.min(...stonesInZone.map(t => t.y ?? 0)) - PADDING
    const maxX = Math.max(...stonesInZone.map(t => (t.x ?? 0) + STONE_W)) + PADDING
    const maxY = Math.max(...stonesInZone.map(t => (t.y ?? 0) + STONE_H)) + PADDING
    const bboxW = maxX - minX
    const bboxH = maxY - minY

    // Zoom to fit: never zoom IN past 1, but zoom OUT if stones spread too wide.
    const zoom = Math.max(MIN_ZOOM, Math.min(1, Math.min(viewportPx.w / bboxW, viewportPx.h / bboxH)))
    const centerX = (minX + maxX) / 2
    const centerY = (minY + maxY) / 2

    setViewport(clampViewport({
      x: centerX - viewportPx.w / 2 / zoom,
      y: centerY - viewportPx.h / 2 / zoom,
      zoom,
    }))
  }
